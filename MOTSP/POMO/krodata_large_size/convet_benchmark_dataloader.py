...
def load_data(num_nodes):
    kroA_data = np.loadtxt('krodata_large_size/2tsp-150/dimacs_%d_a.tsp'%num_nodes, skiprows=6, usecols=(1, 2), delimiter=' ', dtype=float)
    kroB_data = np.loadtxt('krodata_large_size/2tsp-150/dimacs_%d_b.tsp'%num_nodes, skiprows=6, usecols=(1, 2), delimiter=' ', dtype=float)
    ...
    dataset = [ins_AB]
    EPF1 = np.loadtxt('krodata_large_size/2tsp-150/2tsp-150-ExactParetoFront.txt')[:, 1:] / np.array([max_a, max_b])  # AB
